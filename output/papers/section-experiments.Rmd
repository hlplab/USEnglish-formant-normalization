...
To compare the performance of different normalization accounts against listeners' perception, we conducted two small web-based experiments on US English listeners' perception of US English vowels. Both experiments investigate listeners' perception of a single talker. This choice was made so as to not confound questions about formant normalization with questions about talker recognition, and inferences about talker switches [@magnuson-nusbaum2007]. The two experiments employ the same eight-alternative forced-choice vowel categorization task (Figure \@ref(fig:exp-procedure)), and differ only in the whether they employed 'natural' (Experiment 1a) or synthesized stimuli (Experiment 1b). To the best of our knowledge, these two experiments are the first designed to compare normalization accounts against listeners' perception over a larger portion of the monophthong inventory of a language.

(ref:exp-procedure) Screen shot of the eight-alternative forced-choice (8-AFC) task used in both Experiment 1a and 1b.

```{r exp-procedure, echo=FALSE, fig.cap="(ref:exp-procedure)", out.width='40%', fig.show='asis'}
include_and_generate_graphics("figures-to-import/response-grid.png")
```
...
d.test <-
  ...
  select(
    ...
    starts_with("F", ignore.case = F), starts_with("Duration"), starts_with("Exclude"), Response.Vowel, Response, platform,
...
We recruited `r d.test %>% filter(Experiment == "Experiment 1a (natural)") %>% distinct(ParticipantID) %>% nrow()` (Experiment 1a) and `r d.test %>% filter(Experiment == "Experiment 1b (synthesized)") %>% distinct(ParticipantID) %>% nrow()` (Experiment 1b) participants. The majority of these (`r d.test %>% filter(platform == "MTurk") %>% distinct(ParticipantID) %>% nrow()` for each experiment) were recruited from Amazon's Mechanical Turk. However, after exclusions we were left with a relatively low number of participants (for Experiment 1a, `r d.test %>% filter(Experiment == "Experiment 1a (natural)", platform == "MTurk", Exclude_Participant.Reason == "none") %>% distinct(ParticipantID) %>% nrow()`, and for Experiment 1b, `r d.test %>% filter(Experiment == "Experiment 1b (synthesized)", platform == "MTurk", Exclude_Participant.Reason == "none") %>% distinct(ParticipantID) %>% nrow()`). We therefore decided to recruit an additional `r d.test %>% filter(platform == "Prolific") %>% group_by(Experiment) %>% distinct(ParticipantID) %>% nrow()` participants from Prolific (`r d.test %>% filter(platform == "Prolific") %>% distinct(ParticipantID) %>% nrow()` for each experiment; October 2024). Exclusions described below left `r d.test %>% filter(Experiment == "Experiment 1a (natural)", Exclude_Participant.Reason == "none") %>% distinct(ParticipantID) %>% nrow()` and `r d.test %>% filter(Experiment == "Experiment 1b (synthesized)", Exclude_Participant.Reason == "none") %>% distinct(ParticipantID) %>% nrow()` participants for analysis in Experiments 1a and 1b, respectively. Results did not change after inclusion of the new participants from Prolific.
...
# # Make experiment data file after exclusions - code kept for reproducibility
# write_csv(
#   d.test %>%
#     relocate(c(F0, F1, F2, F3, Duration), .after = Item.F2),
#   file =  "../../data/experiment-data/Experiment-1a-1b-after-exclusions.csv")

# Normalize the perception data
d.test %<>%
  group_by(Experiment) %>%
...
# # Make experiment data file after exclusions with normalized cues - code kept for reproducibility
# write_csv(
#   d.test %>%
#     select(-starts_with("overall"), -starts_with("formants")) %>%
#     relocate(c(Item.F1, Item.F2), .after = Item.CorrectResponse.Vowel) %>%
#     relocate(c(Duration, Duration.Assignment, Response.Vowel, Response, platform, fold_type, Talker), .before = Item.F1),
#   file =  "../../data/experiment-data/Experiment-1a-1b-after-exclusions-with-normalization.csv")
...
### Exclusions
We excluded participants who failed to follow instructions and did not wear over-the-ear headphones (as indicated in the post-experiment survey). We also excluded participants with mean (log-transformed) reaction times that were unusually slow or fast (absolute z-score over by-participant means > 3), or if they clearly did not do the task (e.g., by answering randomly). This excluded `r english::as.english(d.test.exclusion %>% filter(Experiment == "Experiment 1a (natural)", Exclude_Participant.Reason != "none") %>% distinct(ParticipantID) %>% nrow())` participants from Experiment 1a and `r english::as.english(d.test.exclusion %>% filter(Experiment == "Experiment 1b (synthesized)", Exclude_Participant.Reason != "none") %>% distinct(ParticipantID) %>% nrow())` from Experiment 1b (for details, see SI \@ref(sec:SI-exclusions)).
...
Participants' categorization responses in Experiments 1a and 1b are shown in Figure \@ref(fig:human-performance), with larger labels indicating recordings that participants agreed on more.^[@shannon1948 response entropy is defined as $H(x) = -\sum_{i=1}^{n} P(x_{i}) \log P(x_{i})$. The maximum possible response entropy for an eight-way response choice is 3 bits, which means that all eight vowels are responded equally often. The minimum response entropy = 0 bits, which means that the same vowel is responded all the time.] We make two observations. The first pertains to the degree of (dis)agreement between the two experiments. The second observation pertains to the degree of (dis)agreement across participants within each experiment.
...
(ref:human-performance) Summary of listeners' categorization responses in Experiments 1a and 1b in F1-F2 space. The vowel label indicates the most frequent response provided across participants on each test location. Size indicates how consistent responses were across participants, with larger symbols indicating more consistent responses (lower entropy). F1-F2 combinations below the gray dashed line are unlikely to be articulated by the same talker.
...
```{r human-confusion, fig.width=base.width*4, fig.height=base.height*4, out.width='90%', fig.cap="(ref:human-confusion)", message=FALSE}
...
p.shifters <- d.shifters %>%
  ...
  ggplot(
    aes(
      ...
      y = `[ɛ] with [æ]`,
      color = `[u] with [ʊ]`)) +
  geom_text(aes(label = listeners), alpha = 1, fontface = 2) +
