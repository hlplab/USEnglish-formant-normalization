...
```{r exp-procedure, echo=FALSE, fig.cap="(ref:exp-procedure)", out.width='40%', fig.align='center', fig.show='asis'}
...
```{r spectrograms, fig.align='center', fig.cap="(ref:spectrograms)", fig.height=base.height*2, echo=FALSE}
...
```{r save-composed-image-with-subcaptions, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center'}
# Generate composed image and read in below
# # List of images and subcaptions
# images <- c(
#   "figures-to-import/spectrogram_heed_1a.png", "figures-to-import/spectrogram_hid_1a.png",
#   "figures-to-import/spectrogram_odd_1a.png", "figures-to-import/spectrogram_hood_1a.png",
#   "figures-to-import/spectrogram_heed.png", "figures-to-import/spectrogram_hid.png",
...
# 
# subcaptions <- c(
#   expression("(a) natural " * italic("heed")), expression("(b) natural " * italic("hid")),
#   expression("(c) natural " * italic("odd")), expression("(d) natural " * italic("hood")),
#   expression("(e) resynthesized " * italic("heed")), expression("(f) resynthesized " * italic("hid")),
#   expression("(g) resynthesized " * italic("hod")), expression("(h) resynthesized " * italic("hood"))
# )
# 
# fig.cap="(ref:spectrograms)"
# 
# # Create individual grobs for each image + caption
# image_grobs <- lapply(seq_along(images), function(i) {
#   img <- grid::rasterGrob(png::readPNG(images[i]), interpolate = TRUE)  # Load image
#   caption <- grid::textGrob(subcaptions[i], x = 0.5, y = 0.5, just = "center", gp = grid::gpar(fontsize = 42))
#   gridExtra::grid.arrange(img, caption, nrow = 2, heights = c(4, 1))  # Combine image + caption
# })
# 
# # Arrange the image-caption grobs in a grid
# composed_figure <- do.call(gridExtra::grid.arrange, c(image_grobs, ncol = 4))
# 
# # Save the composed figure as an image
# png(figure_file("Figure3.png"), width = 3200, height = 1600)
# grid::grid.draw(composed_figure)
# dev.off()
```

```{r spectrograms1, echo=FALSE, fig.align='center', out.width='90%', fig.cap="(ref:spectrograms1)"}
#include_and_generate_graphics(figure_file("Figure3.png"))
```
...
(ref:human-performance) Summary of listeners' categorization responses in Experiments 1a and 1b in F1-F2 space. The vowel label indicates the most frequent response provided across participants on each test location. Size indicates how consistent responses were across participants, which larger symbols indicating more consistent responses (lower entropy). F1-F2 combinations below the gray dashed line are unlikely to be articulated by the same talker.
