...
## Methods {#sec:methods-study1}

### A general-purpose categorization model for $J$-alternative forced-choice categorization tasks {#sec:predict-perception}

(ref:model-perceptual-decision-making) Graphical model of ASP's general categorization framework (adapted for the current purpose from Xie et al., 2023, Figure 4). Here $J=8$ (the eight vowel response options in Experiments 1a and 1b). We use this framework to compare normalization accounts against listeners' categorization responses from Experiments 1a and 1b. Filled gray circles represent variables that are known to the researcher. Empty circles represent latent variables that are not observable. Diamonds represent variable-free processes, annotated with the distributions resulting at that level of the model: $\mathcal{N}$(ormal), Multi(nomial), and $\mathcal{M}$(ixture) distributions.

```{r model-perceptual-decision-making, fig.cap="(ref:model-perceptual-decision-making)", fig.width=10, out.width="95%"}
include_and_generate_graphics("figures-to-import/ASP-graphical-model.pdf")
```
...
```{r io-plot-categories, fig.width=base.width*3.5, fig.height=base.height*2+1/4, out.width='95%', fig.cap="(ref:io-plot-categories)", message=FALSE}
p.io.categories.1a <-
  p.vowel_space %+%
  (io %>%
     filter(
...
```{r prediction-landscapes, fig.cap="(ref:prediction-landscapes)", fig.width=base.width*4, fig.height=base.height*2.5, out.width='95%', message=FALSE, warning=FALSE}
...
```{r study1-best-fit-likelihoods, message=FALSE, warning=FALSE}
d.io.mean_fits <-
  io %>%
...
```{r plot-io-optimal, fig.width=base.width*3.5, fig.height=base.height*3, out.width='95%', fig.cap="(ref:plot-io-optimal)"}
...
p.result_overall +
  ...
  geom_text(
    ...
    aes(
      x = IO.NormalizationType,
      y = ifelse(Experiment == "Experiment 1a (natural)", -1.1, -1.9),
...
Figure \@ref(fig:plot-io-optimal) compares how well the different normalization accounts fit listeners' responses in Experiments 1a and 1b. All accounts performed well above chance guessing (chance per-token log-likelihood in both experiments: $ln(\frac{1}{8})$=`r d.chance.max %>% filter(Experiment == "Experiment 1a (natural)") %>% pull(log_likelihood_chance_scaled)`) but also well below the highest possible performance (in Experiment 1a, per-token log-likelihood = `r d.chance.max %>% filter(Experiment == "Experiment 1a (natural)") %>% pull(log_likelihood_human_scaled)`, in Experiment 1b: `r d.chance.max %>% filter(Experiment == "Experiment 1b (synthesized)") %>% pull(log_likelihood_human_scaled)`).
...
```{r prediction-landscapes-optim-io, fig.cap="(ref:prediction-landscapes-optim-io)", out.width='80%', message=FALSE, warning=FALSE}
if (RESET_MODELS || !file.exists("figures-to-import/p.prediction-landscapes-study1.png")) {
  ...
  p <-
    p.raw / p.johnson / p.nearey2 +
    plot_layout(
      guides = "collect") &
    plot_annotation(
      tag_levels = list(paste(LETTERS[1:4], labels.normalization[c(1, 12, 10)], sep = " - "))) &
    theme(legend.position = "bottom",
          legend.justification = "bottom",
          legend.margin = margin(0, 0, 0, 0, unit = "pt"),
  ...
  ggsave(p, file = "figures-to-import/p.prediction-landscapes-study1.png", width = base.width*3.5, height = base.height*4.5)

}

include_and_generate_graphics("figures-to-import/p.prediction-landscapes-study1.png")
```
