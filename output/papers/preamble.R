...
make_plot_namer <- function(prefix = "Figure", width = 2) {
  counter <- 1L  # Initialize a counter variable to 0
  
  function(x) {
    x2 <- file.path(dirname(x), 
                    paste0(prefix, 
                           formatC(counter, width = width, flag = "0"),
                           ".", 
                           tools::file_ext(x)))
...
include_and_generate_graphics <- function(path, caption = NULL, ...) {
  # Create a temporary R plot using magick
  tmp_plot <- function(file) {
    if (file.exists(file)) {
      p <-
        magick::image_read(file, density = 900) %>%
        magick::image_ggplot()
    } else {
      p <-
        ggplot(NULL) +
        annotate(geom = "text", x = 0, y = 0, label = paste("Figure file", file, "not found!"))
    }
  ...
  if (length(path) == 1) {
    ...
    p.list <- list()
    for (p in path) p.list[[p]] <- tmp_plot(p)
    cowplot::plot_grid(
      plotlist = p.list, align = "hv", axis = "tblr",
      ...)
  }
}
...
knitr::opts_chunk$set(
  ...
  # Name figures after chunk instead:
