...
While it is relatively uncontroversial *that* normalization contributes to robust speech perception, it is still unclear what types of computations this implicates. We address this question for the perception of vowels, which cross-linguistically relies on peaks in the distribution of spectral energy over acoustic frequencies (formants).^[Some hypotheses hold that robust speech perception can be achieved without normalization once it is recognized that listeners have access to much richer acoustic information than the phonetic cues that researchers tend to focus on [for review, see e.g., @strange-jenkins2012]. For vowel recognition, for example, listeners might use more cues than just formants [@hillenbrand2006; @nearey-assmann1986], and/or might use information about the dynamic development of formant trajectories over time rather than just point estimates of formants at the vowel center [e.g., @shankweiler1978]. We briefly return to this in the general discussion.] Vowel perception has long been a focus in research on normalization [e.g., @bladon1984; @fant1975; @gerstman1968; @johnson2020; @joos1948; @lobanov1971; @miller1989; @nearey1978; @nordstrom-lindblom1975; @syrdal-gopal1986; @traunmuller1981; @watt-fabricius2002; @zahorian-jagharghi1991; for review, see @barreda2020], with some reviews citing over 100 competing proposals [@carpenter-govindarajan1993]. Importantly, these accounts differ in the types and complexity of computations they assume to take place during normalization. On the lower end of computational complexity, 'fixed' transformations of the acoustic signal might suffice to achieve invariance from cues to phonetic categories. For example, there is evidence that a transformation of acoustic frequencies (measured in Hz) into the psycho-acoustic Mel-space better describes how listeners perceive differences in the frequency of sine tones [e.g., @fastl-zwicker2007; @stevens-volkmann1940; for a critique, see @greenwood1997; @siegel1965]. <!-- TO DO: check what did those experiments show? -->It is thus possible that cross-talker variability in vowel pronunciations is effectively reduced when formants are represented in Mel, rather than Hz. Similar arguments have been made about other psycho-acoustic transformations [e.g., Bark, @traunmuller1990; ERB, @glasberg-moore1990; or semitones, @fant2002]. While the Mel scale was developed as a model of human pitch perception, other psycho-acoustic transformations were developed with other applications in mind. For instance, semitones transformation is generally linked to musical pitch [@balzano1982] while the Bark and ERB scales were proposals of human auditory frequency selectivity [@glasberg-moore1990; @zwicker1957; @zwicker-terhardt1980]. Transformations like these have no degrees of freedom that listeners would need to estimate from the acoustic input. We therefore refer to them as *estimation-free* transformation. Several of these accounts share that they log-transform acoustic frequencies---in line with neurophysiological evidence that the auditory representations in the brain seem to follow a roughly logarithmic organization, so that auditory perception is (up to a point) more sensitive to differences between lower frequencies than to the same difference between higher frequencies [e.g., @merzenich1975; for review, see @saenz-langers2014]. If such estimation-free psycho-acoustic transformations are sufficient for formant normalization, this would offer a particularly parsimonious account of vowel perception as listeners would not have to infer talker-specific properties.
