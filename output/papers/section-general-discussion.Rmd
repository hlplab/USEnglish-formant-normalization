...
Finally, the evaluation of normalization accounts in the present study shares with all previous work [e.g., @apfelbaum-mcmurray2015; @cole2010; @mcmurray-jongman2011; @barreda2021; @nearey1989;  @richter2017] another simplifying assumption that is clearly wrong: the assumption that listeners *know* the talker-specific formant properties required for normalization. Specifically, we normalized the input for each ideal observer using the maximum likelihood estimates of the normalization parameters over all stimuli for the respective experiment. For example, for the evaluation of the ideal observer trained on Lobanov normalized formants against listeners' responses in Experiment 1a, we used the formant means and standard deviations of the stimuli used in Experiment 1a to normalize F1 and F2. While this follows previous work, it constitutes a problematic assumption for the evaluation of extrinsic normalization accounts. For extrinsic accounts, the approach adopted here would seem to entail the ability to predict the future: even on the first trial of the experiment, the input to the ideal observers were formants that were normalized based on the normalization parameters estimated over the acoustic properties of *all* stimuli. Listeners instead need to *incrementally infer* talker-specific properties from the speech input [@nearey-assmann2007; @xie2023]. An important avenue for future research is thus the development and evaluation of incremental normalization accounts.

The present data only allow an initial, rather tentative, look at this question. For example, for Experiment 1a, for which each trial had a known correct answer (the vowel intended by the talker), we can assess whether participants' recognition accuracy improved across trials. Figure \@ref(fig:trial-accuracies)A suggests that this was indeed the case: the non-parametric listeners' average recognition accuracy improved over the course of the experiment from about 65% to 88%, with most of the improvements occurring during the first ten trials. This result should, however, be interpreted with great caution. Experiment 1a was not designed to address this question, and the number of participants we recruited is too small to average out differences in the distribution of stimuli across trials---leaving open whether the observed changes in recognition accuracy reflect item-specific effects, rather than improvements across trials.

Figure \@ref(fig:trial-accuracies)B shows how the fit of the best-fitting normalization model changes across trials. Given that our evaluation of normalization accounts assumed that the normalization parameters were already known on the first trial of the experiment, we would expect that the likelihood of listeners' responses under a normalization model would improve the more input listeners have received (i.e., as the simplifying assumptions of our evaluation become increasingly more plausible). For Experiment 1a, this indeed appears to be the case. However, just as the improvements in listeners' accuracy in Figure \@ref(fig:trial-accuracies)A might reflect item-specific effects, the improved fit of the normalization model in Figure \@ref(fig:trial-accuracies)B also might reflect item-specific effects, rather than incremental improvements in the fit of the normalization model. In line with this concern, no incremental improvement in the fit of the normalization model was observed for Experiment 1b. The present study thus leaves open whether we simply do not have enough data to address this question (without changes in our modeling approach), whether incremental normalization is more or less instantaneous (in which no improvements across trials are expected), and whether the answer to the these questions differ between the natural and synthesized stimuli in Experiments 1a and 1b.

(ref:trial-accuracies) **Panel A**: Changes across trials in listeners' average accuracy in recognizing the vowel intended by the talker in Experiment 1a, averaged across items and participants (black line). Blue line shows a non-parametric smooth fit predicting accuracy from (log-transformed) trial order, with 95% CIs. **Panel B**: Log-likelihood of listeners' responses under the best-fitting normalization account at each trial, averaged across items and participants (Johnson's uniform scaling for Experiment 1a and Nearey's uniform scaling for Experiment 1b). Blue line shows a non-parametric smooth fit predicting log-likelihood from (log-transformed) trial order, with 95% CIs.

```{r trial-accuracies,  fig.width=base.width*4, fig.height=base.height*3, out.width='90%', fig.cap="(ref:trial-accuracies)", fig.pos="!ht", message=FALSE, warning=FALSE}
...
p.trial.accuracy <- 
  ...
  geom_path() +
  stat_smooth(method = "gam", formula = y ~ s(log(x))) +
  scale_y_continuous("mean accuracy of participants' responses", limits = c(20, 100), breaks = c(20, 40, 60, 80, 100)) + 
...
p.predicted.trial.likelihood <- 
  # Get perception data
  d.predicted.trial.likelihood %>%
  select(Experiment, IO.cue_normalization, IO.crossvalidation_group, likelihood_up_to_constant_byItem, x, ItemID, Trial, Response.Vowel) %>%
  ...
  #     group_by(Experiment) %>%
  #     group_map(
  #       .f = ~ gam(likelihood ~ s(log(Trial)), data = .) %>% 
  #         broom::tidy()) %>% 
  #     reduce(bind_rows) %>%
  #     mutate(
  #       Experiment = unique(d.predicted.trial.likelihood$Experiment),
  #       Star = ifelse(p.value < .05, "*", "n.s.")),
  #   aes(label = Star), x = -1, y = 1) +
  scale_x_continuous(breaks = c(1, 10, 100)) +
...
p.trial.accuracy + p.predicted.trial.likelihood + 
  ...
  plot_layout(guides = "collect", widths = c(.94,2)) &
